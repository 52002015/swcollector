							...
							for _, lastifStat := range *lastIfStatList {
								if ifStat.IfIndex == lastifStat.IfIndex {
									...
									if limitCheck(IfHCInBroadcastPkts, broadcastlimit) {
										L = append(L, GaugeValueIp(ts, ip, "snmp.if.InBroadcastPkt", IfHCInBroadcastPkts, ifNameTag, ifIndexTag))
									} else {
										log.Println(ip, ifNameTag, "snmp.if.InBroadcastPkt ", "out of range, value is ", IfHCInBroadcastPkts, "Limit is ", broadcastlimit)
									...
									if limitCheck(IfHCOutBroadcastPkts, broadcastlimit) {
										L = append(L, GaugeValueIp(ts, ip, "snmp.if.OutBroadcastPkt", IfHCOutBroadcastPkts, ifNameTag, ifIndexTag))
									} else {
										log.Println(ip, ifNameTag, "snmp.if.OutBroadcastPkt ", "out of range, value is ", IfHCOutBroadcastPkts, "Limit is ", broadcastlimit)
							...
							for _, lastifStat := range *lastIfStatList {
								if ifStat.IfIndex == lastifStat.IfIndex {
									...
									if limitCheck(IfHCInMulticastPkts, multicastlimit) {
										L = append(L, GaugeValueIp(ts, ip, "snmp.if.InMulticastPkt", IfHCInMulticastPkts, ifNameTag, ifIndexTag))
									} else {
										log.Println(ip, ifNameTag, "snmp.if.InMulticastPkt ", "out of range, value is ", IfHCInMulticastPkts, "Limit is ", multicastlimit)
									...
									if limitCheck(IfHCOutMulticastPkts, multicastlimit) {
										L = append(L, GaugeValueIp(ts, ip, "snmp.if.OutMulticastPkt", IfHCOutMulticastPkts, ifNameTag, ifIndexTag))
									} else {
										log.Println(ip, ifNameTag, "snmp.if.OutMulticastPkt ", "out of range, value is ", IfHCOutMulticastPkts, "Limit is ", multicastlimit)
							...
							for _, lastifStat := range *lastIfStatList {
								if ifStat.IfIndex == lastifStat.IfIndex {
									...
									if limitCheck(IfInDiscards, discardlimit) {
										L = append(L, GaugeValueIp(ts, ip, "snmp.if.InDiscards", IfInDiscards, ifNameTag, ifIndexTag))
									} else {
										log.Println(ip, ifNameTag, "snmp.if.InDiscards ", "out of range, value is ", IfInDiscards, "Limit is ", discardlimit)
									...
									if limitCheck(IfOutDiscards, discardlimit) {
										L = append(L, GaugeValueIp(ts, ip, "snmp.if.OutDiscards", IfOutDiscards, ifNameTag, ifIndexTag))
									} else {
										log.Println(ip, ifNameTag, "snmp.if.OutDiscards ", "out of range, value is ", IfOutDiscards, "Limit is ", discardlimit)
							...
							for _, lastifStat := range *lastIfStatList {
								if ifStat.IfIndex == lastifStat.IfIndex {
									...
									if limitCheck(IfInErrors, errorlimit) {
										L = append(L, GaugeValueIp(ts, ip, "snmp.if.InErrors", IfInErrors, ifNameTag, ifIndexTag))
									} else {
										log.Println(ip, ifNameTag, "snmp.if.InErrors ", "out of range, value is ", IfInErrors, "Limit is ", errorlimit)
									...
									if limitCheck(IfOutErrors, errorlimit) {
										L = append(L, GaugeValueIp(ts, ip, "snmp.if.OutErrors", IfOutErrors, ifNameTag, ifIndexTag))
									} else {
										log.Println(ip, ifNameTag, "snmp.if.OutErrors ", "out of range, value is ", IfOutErrors, "Limit is ", errorlimit)
							...
							for _, lastifStat := range *lastIfStatList {
								if ifStat.IfIndex == lastifStat.IfIndex {
									...
									if limitCheck(IfInUnknownProtos, unknownProtoslimit) {
										L = append(L, GaugeValueIp(ts, ip, "snmp.if.InUnknownProtos", IfInUnknownProtos, ifNameTag, ifIndexTag))
									} else {
										log.Println(ip, ifNameTag, "snmp.if.InUnknownProtos ", "out of range, value is ", IfInUnknownProtos, "Limit is ", unknownProtoslimit)
							...
							for _, lastifStat := range *lastIfStatList {
								if ifStat.IfIndex == lastifStat.IfIndex {
									...
									if limitCheck(IfOutQLen, outQlenlimit) {
										L = append(L, GaugeValueIp(ts, ip, "snmp.if.OutQLen", IfOutQLen, ifNameTag, ifIndexTag))
									} else {
										log.Println(ip, ifNameTag, "snmp.if.OutQLen ", "out of range, value is ", IfOutQLen, "Limit is ", outQlenlimit)
							...
							for _, lastifStat := range *lastIfStatList {
								if ifStat.IfIndex == lastifStat.IfIndex {
									...
									if limitCheck(IfHCInUcastPkts, pktlimit) {
										L = append(L, GaugeValueIp(ts, ip, "snmp.if.InPkts", IfHCInUcastPkts, ifNameTag, ifIndexTag))
									} else {
										log.Println(ip, ifNameTag, "snmp.if.InPkts ", "out of range, value is ", IfHCInUcastPkts, "Limit is ", pktlimit)
									...
									if limitCheck(IfHCOutUcastPkts, pktlimit) {
										L = append(L, GaugeValueIp(ts, ip, "snmp.if.OutPkts", IfHCOutUcastPkts, ifNameTag, ifIndexTag))
									} else {
										log.Println(ip, ifNameTag, "snmp.if.OutPkts ", "out of range, value is ", IfHCOutUcastPkts, "Limit is ", pktlimit)
						...
						for _, lastifStat := range *lastIfStatList {
							if ifStat.IfIndex == lastifStat.IfIndex {
								...
								if limitCheck(IfHCInOctets, speedlimit) {
									L = append(L, GaugeValueIp(ts, ip, "snmp.if.In", IfHCInOctets, ifNameTag, ifIndexTag))
									if ifStat.IfSpeed > 0 {
										InSpeedPercent := 100 * IfHCInOctets / float64(ifStat.IfSpeed)
										L = append(L, GaugeValueIp(ts, ip, "snmp.if.InSpeedPercent", InSpeedPercent, ifNameTag, ifIndexTag))
									}
								} else {
									log.Println(ip, ifNameTag, "snmp.if.In ", "out of range, value is ", IfHCInOctets)
								...
								if limitCheck(IfHCOutOctets, speedlimit) {
									L = append(L, GaugeValueIp(ts, ip, "snmp.if.Out", IfHCOutOctets, ifNameTag, ifIndexTag))
									if ifStat.IfSpeed > 0 {
										OutSpeedPercent := 100 * IfHCOutOctets / float64(ifStat.IfSpeed)
										L = append(L, GaugeValueIp(ts, ip, "snmp.if.OutSpeedPercent", OutSpeedPercent, ifNameTag, ifIndexTag))
									}
								} else {
									log.Println(ip, ifNameTag, "snmp.if.Out ", "out of range, value is ", IfHCOutOctets)
